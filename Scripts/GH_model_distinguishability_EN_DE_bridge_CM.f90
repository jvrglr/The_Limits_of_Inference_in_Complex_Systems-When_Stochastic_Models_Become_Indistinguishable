program Model_distinguishability_EN_DE
    ...
    integer*8 :: ii,jj,kk,ll,meas,n_short,seed,len_dts,len_n_rows,n_measures,realizs,select_model,n_params_target_model,N_bridges
    ...
    call logspace(start = -1.0d0, end = 2.0d0, num = len_dts, base = 10.0d0, result = dts)
    n_rows = [100]
    ! dts = [0.26d0]
    ! n_rows = [100]
    if(len_n_rows==1) then
        filename = "data/P_distinguish_EN_DE_target_"//trim(model)//"_bridge_CM_proteus_NB_"//trim(str8(n_rows(1)))//"_V2.dat"
    else
        filename = "data/P_distinguish_EN_DE_target_"//trim(model)//"_bridge_CM_proteus_V2.dat"
    endif
    print *
    ...
    do jj = 1, len_n_rows, 1
        ...
        do kk = 1, len_dts, 1
            ...
            do ll = 1, realizs, 1
                ...
                target_model = "EN"
                call Evaluate_log_likelihood_time_series_w_bridge_CM(ts,xs,n_measures,target_model,params_target_model,n_params_target_model,dt_bridge,N_bridges,model_bridge,LK_EN)
                ...
                target_model = "DE"
                call Evaluate_log_likelihood_time_series_w_bridge_CM(ts,xs,n_measures,target_model,params_target_model,n_params_target_model,dt_bridge,N_bridges,model_bridge,LK_DE)
